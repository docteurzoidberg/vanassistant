...
public:
    VerticalTextAnimator(IDrzEngine* engine, const font* fnt, uint8_t lineHeight, float typeSpeed, float pauseTime, float cursorBlinkRate, int rectX, int rectY, int rectWidth, int rectHeight, int cursorWidth, int cursorHeight)
        : engine(engine), lineHeight(lineHeight), typeSpeed(typeSpeed), pauseTime(pauseTime), cursorBlinkRate(cursorBlinkRate),
...
private:
    ...
    void DrawCursor(int x, int y, const std::string& toDraw, color color) {
      if (cursorVisible && (isTyping || (!isTyping && !textQueue.empty()))) {
        ...
        auto cursorWidth = 8;
        auto cursorHeight = fnt->yAdvance - 4;

        int cursorX = x + textSize.w+2;
        int cursorY = y - fnt->yAdvance +6;
        engine->FillRect(cursorX, y, cursorWidth, cursorHeight, color); // Draw the cursor using specified width and height
      }
    }
