...
public:
  ...
  bool OnUserUpdate(float fElapsedTime) override {
    ...
    if (guiSliderFuel->bHeld) {
      ...
      if (dbgGuiUpdates)
        std::cout << "sliderFuel changed" << std::endl;
      guiLabelFuelValue->sText =
          std::to_string((int)((data.fuelgauge / 255.0f) * 100.0f)) + "%";
    ...
    if (guiSliderSpeed->bHeld) {
      ...
      if (dbgGuiUpdates)
        std::cout << "sliderSpeed changed" << std::endl;
    ...
    if (guiSliderBat->bHeld) {
      ...
      if (dbgGuiUpdates)
        std::cout << "sliderBat changed" << std::endl;
      guiLabelBatValue->sText =
          strRoundUpToNDecimals(data.battery / 10.0f, 1) + "V";
      bGUIDataUpdated = true;
    }
    ...
    if (guiSliderRPM->bHeld) {
      ...
      if (dbgGuiUpdates)
        std::cout << "sliderRPM changed" << std::endl;
    ...
    if (guiSliderCoolantTemp->bHeld) {
      ...
      if (dbgGuiUpdates)
        std::cout << "sliderCoolantTemp changed" << std::endl;
      guiLabelCoolantTempValue->sText =
          strRoundUpToNDecimals(data.coolant_temp / 10.0f, 1) + "C";
      bGUIDataUpdated = true;
    }

    if (guiTextBoxOdometer->sText != sGuiTextBoxOdometerOldValue) {
      std::cerr << "txtOdometer changed. oldValue = "
                << sGuiTextBoxOdometerOldValue
                << " newValue = " << guiTextBoxOdometer->sText << std::endl;
      try {
    ...
    if (guiTextBoxTrip->sText != sGuiTextBoxTripOldValue) {
      std::cerr << "txtTrip changed. oldValue = " << sGuiTextBoxTripOldValue
                << " newValue = " << guiTextBoxTrip->sText << std::endl;
      try {
    ...
    if (guiCheckBoxPreheat->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkPreheat pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxNotCharging->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkNotCharging pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxOil->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkOil pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxTurnSignals->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkTurnSignals pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxHighBeam->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkHighBeam pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxLowBeam->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkLowBeam pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxWarnings->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkWarnings pressed" << std::endl;
      bGUIDataUpdated = true;
    }
    ...
    if (guiCheckBoxProblem->bPressed) {
      ...
      if (dbgGuiUpdates)
        std::cout << "chkProblem pressed" << std::endl;
      bGUIDataUpdated = true;
    }
...
private:
  ...
  void LoadSprites() {
    sprCounterSpritesheet =
        new olc::Sprite("serial-gui/sprites/counterdigits.png");
    sprInstrumentMask =
        new olc::Sprite("serial-gui/sprites/dashboard_facemask.png");
    sprLampPreheat.Load("serial-gui/sprites/lamp_preheat.png");
    sprLampNotCharging.Load("serial-gui/sprites/lamp_notcharging.png");
    sprLampOil.Load("serial-gui/sprites/lamp_oil.png");
    sprLampTurnSignals.Load("serial-gui/sprites/lamp_turnsignals.png");
    sprLampHighBeam.Load("serial-gui/sprites/lamp_highbeam.png");
    sprLampLowBeam.Load("serial-gui/sprites/lamp_lowbeam.png");
    sprLampWarnings.Load("serial-gui/sprites/lamp_warnings.png");
    sprLampProblem.Load("serial-gui/sprites/lamp_problem.png");
  }
  ...
  void SetupGUI() {
    ...
    guiLabelFuel =
        new olc::QuickGUI::Label(guiManager, "Fuel:", {lx, ly}, {70.0f, 16.0f});
    guiLabelFuelMinValue = new olc::QuickGUI::Label(
        guiManager, "0", {lx + 156, ly}, {10.0f, 16.0f});
    guiLabelFuelMaxValue = new olc::QuickGUI::Label(
        guiManager, "100%", {lx + 252, ly}, {10.0f, 16.0f});
    guiLabelFuelValue = new olc::QuickGUI::Label(guiManager, "50%",
                                                 {lx + 74, ly}, {70.0f, 16.0f});

    guiLabelSpeed = new olc::QuickGUI::Label(
        guiManager, "Speed:", {lx, ly + 18}, {70.0f, 16.0f});
    guiLabelSpeedMinValue = new olc::QuickGUI::Label(
        guiManager, "0", {lx + 156, ly + 18}, {10.0f, 16.0f});
    guiLabelSpeedMaxValue = new olc::QuickGUI::Label(
        guiManager, "140km/h", {lx + 252, ly + 18}, {10.0f, 16.0f});
    guiLabelSpeedValue = new olc::QuickGUI::Label(
        guiManager, "88km/h", {lx + 74, ly + 18}, {70.0f, 16.0f});

    guiLabelBat = new olc::QuickGUI::Label(guiManager, "Bat:", {lx, ly + 36},
                                           {70.0f, 16.0f});
    guiLabelBatMinValue = new olc::QuickGUI::Label(
        guiManager, "0", {lx + 156, ly + 36}, {10.0f, 16.0f});
    guiLabelBatMaxValue = new olc::QuickGUI::Label(
        guiManager, "20.0 V", {lx + 252, ly + 36}, {10.0f, 16.0f});
    guiLabelBatValue = new olc::QuickGUI::Label(
        guiManager, "13.4V", {lx + 74, ly + 36}, {70.0f, 16.0f});

    guiLabelRPM = new olc::QuickGUI::Label(guiManager, "RPM:", {lx, ly + 54},
                                           {70.0f, 16.0f});
    guiLabelRPMMinValue = new olc::QuickGUI::Label(
        guiManager, "0", {lx + 156, ly + 54}, {10.0f, 16.0f});
    guiLabelRPMMaxValue = new olc::QuickGUI::Label(
        guiManager, "7000RPM", {lx + 252, ly + 54}, {10.0f, 16.0f});
    guiLabelRPMValue = new olc::QuickGUI::Label(
        guiManager, "3000RPM", {lx + 74, ly + 54}, {70.0f, 16.0f});

    guiLabelCoolantTemp = new olc::QuickGUI::Label(
        guiManager, "Water Temp:", {lx, ly + 72}, {70.0f, 16.0f});
    guiLabelCoolantTempMinValue = new olc::QuickGUI::Label(
        guiManager, "0", {lx + 156, ly + 72}, {10.0f, 16.0f});
    guiLabelCoolantTempMaxValue = new olc::QuickGUI::Label(
        guiManager, "140C", {lx + 252, ly + 72}, {10.0f, 16.0f});
    guiLabelCoolantTempValue = new olc::QuickGUI::Label(
        guiManager, "90C", {lx + 74, ly + 72}, {70.0f, 16.0f});

    guiLabelOdometer = new olc::QuickGUI::Label(
        guiManager, "Odometer:", {lx, ly + 90}, {70.0f, 16.0f});
    guiLabelOdemeterValue = new olc::QuickGUI::Label(
        guiManager, odometerText, {lx + 74, ly + 90}, {70.0f, 16.0f});

    guiLabelTrip = new olc::QuickGUI::Label(guiManager, "Trip:", {lx, ly + 108},
                                            {70.0f, 16.0f});
    guiLabelTripValue = new olc::QuickGUI::Label(
        guiManager, tripText, {lx + 74, ly + 108}, {70.0f, 16.0f});

    // Sliders

    guiSliderFuel = new olc::QuickGUI::Slider(guiManager, {lx + 180, ly + 8},
                                              {lx + 240, ly + 8}, 0, 255, 128);
    guiSliderSpeed = new olc::QuickGUI::Slider(guiManager, {lx + 180, ly + 26},
                                               {lx + 240, ly + 26}, 0, 140, 88);
    guiSliderBat = new olc::QuickGUI::Slider(guiManager, {lx + 180, ly + 44},
                                             {lx + 240, ly + 44}, 0, 200, 123);
    guiSliderRPM = new olc::QuickGUI::Slider(guiManager, {lx + 180, ly + 62},
                                             {lx + 240, ly + 62}, 0, 100, 30);
    guiSliderCoolantTemp = new olc::QuickGUI::Slider(
        guiManager, {lx + 180, ly + 80}, {lx + 240, ly + 80}, 0, 1400, 55);

    // TextBoxes

    guiTextBoxOdometer = new olc::QuickGUI::TextBox(
        guiManager, odometerText, {lx + 171, ly + 90}, {80.0f, 16.0f});
    guiTextBoxTrip = new olc::QuickGUI::TextBox(
        guiManager, tripText, {lx + 171, ly + 108}, {80.0f, 16.0f});

    // CheckBoxes

    guiCheckBoxPreheat = new olc::QuickGUI::ImageCheckBox(
        guiManager, sprLampPreheat, false, {lx - 2, ly + 140}, {24.0f, 24.0f});
    guiCheckBoxNotCharging =
        new olc::QuickGUI::ImageCheckBox(guiManager, sprLampNotCharging, false,
                                         {lx - 2, ly + 164}, {24.0f, 24.0f});
    guiCheckBoxOil = new olc::QuickGUI::ImageCheckBox(
        guiManager, sprLampOil, false, {lx - 2, ly + 188}, {24.0f, 24.0f});
    guiCheckBoxWarnings = new olc::QuickGUI::ImageCheckBox(
        guiManager, sprLampWarnings, false, {lx - 2, ly + 212}, {24.0f, 24.0f});

    guiCheckBoxTurnSignals =
        new olc::QuickGUI::ImageCheckBox(guiManager, sprLampTurnSignals, false,
                                         {lx + 290, ly + 140}, {24.0f, 24.0f});
    guiCheckBoxHighBeam =
        new olc::QuickGUI::ImageCheckBox(guiManager, sprLampHighBeam, false,
                                         {lx + 290, ly + 164}, {24.0f, 24.0f});
    guiCheckBoxLowBeam =
        new olc::QuickGUI::ImageCheckBox(guiManager, sprLampLowBeam, false,
                                         {lx + 290, ly + 188}, {24.0f, 24.0f});
    guiCheckBoxProblem =
        new olc::QuickGUI::ImageCheckBox(guiManager, sprLampProblem, false,
                                         {lx + 290, ly + 212}, {24.0f, 24.0f});
  ...
  /*
     ...
     { ProcessSayTextPacket(&data); } else { std::cerr << "Failed to read
     J7SayTextPacketData" << std::endl;
            }
            break;
          }
          case J7_DASHBOARD: {
            J7DashboardPacketData data;
            if(serialProtocol->ReadJ7DashboardPacketData(serial->read_buf,
     &data)) { ProcessDashboardPacket(&data); } else { std::cerr << "Failed to
     read J7DashboardPacketData. header.len=" << header.len << " bytesRead=" <<
     bytesRead << std::endl;
            }
            break;
          }
  ...
  */
  ...
  void LogJ7DashboardPacketData(J7DashboardPacketData data, int way = 0) {
    ...
    std::cout << wayPrefix << (way ? "Sent: " : "Received: ")
              << "J7DashboardPacketData" << std::endl;
    ...
    std::string strFuel =
        "Fuel: " + std::to_string((int)((data.fuelgauge / 255.0f) * 100.0f)) +
        "%";
    std::string strBat =
        "Bat: " + strRoundUpToNDecimals(data.battery / 10.0f, 1) + "V";
    std::string strSpeed = "Speed: " + std::to_string(data.speed) + "km/h";
    std::string strRPM = "RPM: " + std::to_string(data.rpm * 100) + "RPM";
    std::string strCoolantTemp =
        "Coolant temp: " + strRoundUpToNDecimals(data.coolant_temp / 10.0f, 1) +
        "C";
    std::string strOdometer = "Odometer: " + std::to_string(data.odometer);
    std::string strTrip =
        "Trip: " + strRoundUpToNDecimals(data.trip / 10.0f, 1);
    std::string strLampPreheat =
        "Lamp preheat: " + std::to_string(data.lamp_preheat);
    std::string strLampNotCharging =
        "Lamp not charging: " + std::to_string(data.lamp_notcharging);
    std::string strLampOil = "Lamp oil: " + std::to_string(data.lamp_oil);
    std::string strLampTurnSignals =
        "Lamp turn signals: " + std::to_string(data.lamp_turnsignals);
    std::string strLampHighBeam =
        "Lamp high beam: " + std::to_string(data.lamp_highbeam);
    std::string strLampLowBeam =
        "Lamp low beam: " + std::to_string(data.lamp_lowbeam);
    std::string strLampWarnings =
        "Lamp warnings: " + std::to_string(data.lamp_warnings);
    std::string strLampProblem =
        "Lamp problem: " + std::to_string(data.lamp_problem);
  ...
  void LogJ7IMUPacketData(J7IMUPacketData data, int way = 0) {
    std::string wayPrefix = way ? "->" : "<-";
    std::cout << wayPrefix << (way ? "Sent: " : "Reveived: ")
              << "J7IMUPacketData" << std::endl;
  ...
  void SendJ7DashboardData() {
    ...
    if (throttleData && elapsedMs < (int)(fSendDataEvery * 1000)) {
      // std::cout << "data already sent " << elapsedMs << "ms ago, not sending"
      // << std::endl;
      return;
    }
